        ...
        {videos.map((video, num) => (
          <tr className="table_row" key={num}>
            ...
            <td className="table_cell">
              {onAir && num === 0 ? <GoOnStreamTimer videoLength={video.length}/> : video.length}
            </td>
            <td className="table_cell">
    ...
    <table className="table videoTable">
      <thead className="headRow">
        <tr>
          ...
          <th className="table_cell headCell" colSpan="3">Количество зрители</th>
      ...
      <tbody>
        {streams.map((stream, num) => (
          <tr className="table_row" key={num}>
            ...
            <td className="table_cell">
              {stream.games.map((game, gameNum) => <p className="table_cell_gameName" key={num}>{game}</p>)}
            </td>
    ...
    {videos.length === 0 ? null : (
      <div className="block wideBlock">
        <div className="tableTitle_container">
          <h1 className="tableTitle">Архивные видео</h1>
          <h1 className="tableTitle">Список трансляций</h1>
        </div>
        <StreamsTable streams={streams}/>
        <VideoTable videos={videos} onAir={onAir}/>
      </div>
    )}
  </Fragment>
)};
