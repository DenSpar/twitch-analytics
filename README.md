...
8) написал функции по удалению стримера из основного списка и удалению статы
8.1) проверить кейс такого стримера нет
8.2) проверить кейс если у стримера идет стрим
9) перенес alreadyExistStream в apiHandlers + отделил функцию по поиску текущих стримов
10) написал функцию для проверки если у удаляемого стримера идет стрим + потом написать нормальные ответы для функций

дальше:
+ функция удаления стримеров + должна отписываться от вебхуков + должна ходить во все БД и там все подчищать
+ отдельная функция удаления записаной статы стримеров, можно с таймером через час
+ добавление стримеров в БД - в фронт должен возвращаться новый стример + опопвещение в алерт
...
+ у rxnexus у стрима 02.12.2020 10:58:18 нет зрителей