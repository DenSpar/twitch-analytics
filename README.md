...
дальше:
+ добавление стримеров в БД проводить через интерфейс приложения + должна происходить подписка на webhooks стримера + появляться объект в бд
+ при добавление нового стримера - разделить по разрядам подписчиков
+ функция удаления стримеров + должна отписываться + должна ходить во все БД
