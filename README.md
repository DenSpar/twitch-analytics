...
49) название колонки "количество зрителей" отцентрировать
50) в описание стримера добавить средний и макс онлайн
40?) на фронте отвалилась лампочка о стриме на странице стримера
46) автоматически изменять размер аватарки
44) document.title = newStreamerObj.name;
52) наверно пронумеровать игры в ячейке
53) ховеры и активные классы для кнопок списков
