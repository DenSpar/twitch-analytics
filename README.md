...
1) добавил в бэк функцию searchChannelByName + добавил обработчик апи-запроса на поиск + изменил фронт
2.0) проверить что в req.body - посмотреть что там - числа не должны быть строкой. Если норм расскоментровать функция добавления канала
2) функция создания канала + функция добавления канала в БД
3) refreshLiveStreams разделил на неск модулей + перенес
3.1) проверить кейс если идет стрим
4) функция подписки нового канала на вебхуки
5) проверить что приходит при поиске каналов -> если все норм - менять фронт под новые переменные
6) функция поиска в бэке
7) перенес checkWebHooks

дальше:
+ функция удаления стримеров + должна отписываться от вебхуков + должна ходить во все БД и там все подчищать
+ отдельная функция удаления записаной статы стримеров
+ добавление стримеров в БД - в фронт должен возвращаться новый стример + алерт
+ проверить кейс такого стримера нет
+ проверить кейс если у стримера идет стрим
+ в таблице поиска не должно остаться ховера строк
+ alerts-fix - сделать название канала жирным. Вместо (<strong>{newStreamer.name}</strong>) вставляется [object Object]
+ подписка на вебхуки - 1 минута - исправить

+ что-то не так если стример заканчивает 1 стрим и сразу начинает следующий, возможно из-за не своевременного обновления записи в БД живых стримов. А при обновление стрима в коллекции запускается функция записи? fuck - после апдейта стрима в коллекцие текущих стримов, приходит вебхук-> проверяет есть ли такой стрим в коллекции - есть! значит записывать не надо fuck. в recStream есть функция alreadyExistStream? - лучше убрать
+ предусмотреть функцию, которая будет проверять коллекцию текущих стримов на актуальность