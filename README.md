...
проверить:
0) (после обновления) апи-запрос /api/streamerfromlist/:id
1) поиск:
    -) функция поиска в бэке
    а) добавил в бэк функцию searchChannelByName + добавил обработчик апи-запроса на поиск + изменил фронт
    б) проверить что приходит при поиске каналов -> если все норм - менять фронт под новые переменные
2) добавление канала:
    а) проверить что в req.body - посмотреть что там - числа не должны быть строкой. Если норм расскоментровать функцию добавления канала
    -) функция создания канала + функция добавления канала в БД
    -) refreshLiveStreams разделил на неск модулей + перенес
    б) проверить кейс если идет стрим
    в) функция подписки нового канала на вебхуки
    +) подписка на вебхуки - 1 минута - исправить
3) удаление канала:
    -) перенес checkWebHooks
    -) написал функции по удалению стримера из основного списка и удалению статы
    а) проверить кейс такого стримера нет
    б) проверить кейс если у стримера идет стрим
    -) перенес alreadyExistStream в apiHandlers + отделил функцию по поиску текущих стримов
    +) написал функцию для проверки если у удаляемого стримера идет стрим + потом написать нормальные ответы для функций
    -) поправил обработчик ответов вебхуков - теперь разные сообщения в консоли для подписки и отписки
    -) написал запрос для отписки о вебхука

дальше:
+ добавление стримеров в БД - в фронт должен возвращаться новый стример + оповещение в алерт
+ в таблице поиска не должно остаться ховера строк
+ alerts-fix - сделать название канала жирным. Вместо (<strong>{newStreamer.name}</strong>) вставляется [object Object]  --  можно разделить алерты по типам


+ у rxnexus у стрима 02.12.2020 10:58:18 нет зрителей - приходят null, вроде этот кейс уже учтен?