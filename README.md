...
09.12.20
1) fix удаления стрима из БД через апи, фикс бэка и фронта
2) fix в таблице стримов в массиве указанных игр могут быть пустые строки - "". Чтобы в таблицу не добавлять пустую строку как пункт списка с играми - дописал условие на фронте
3) написал функцию myLogger для логирования в файл, пока записывается в блокнот на дирректорию выше папки back_end

10.12.20
1) подкорректировал myLogger - теперь работает нормально

14.12.20
1) так как pm2 и так логирует сообщения, то в моей функции myLogger нет необходимости - удаляю функцию
2) сделал файл formatedLog.js - так и сейчас будет проще исправить логи, и потом легче будет вносить изменения. +  добавил даты и уровни в логи, чтобы проще было что-то искать в логах. Пока исправил только в index.js. Если все будет норм, то переделаю в остальных файлах
3) логи работают норм, можно переписывать остальные файлы
4) fix recStreamStat - теперь если стрима для записи нет - упоминание о нем будет стерто из БД

15.12.20
1) fix checkWebHooks - нашел ошибки в алгоритме

19.12.20
1) добавил formatedLog во все файлы
2) сделал отдельную кнопку на странице дев для получения списка подписок из twitch-api
3) удалил лишние console.log из фронт части
4) удалил ненужные функции из фронта
5) перенес папку twitchApiRequests на фронте и закоментировал заглушки для тестирования на localhost
6) удалил ненужные заметки в бэке

проверить:
0) 17.01 собрал билд для фронта. Сделать бэкап -> залить на сервер -> если все работает в п5
3) заменил титулы страниц
4) fix recStreamStat - теперь если стрима для записи нет - упоминание о нем будет стерто из БД
5) перенес папку twitchApiRequests на фронте и закоментировал заглушки для тестирования на localhost. Если все норм - удалить закомментированное в файлах: SearchChannel, DashBoard, Streamer