...
24) скрипт проверки если этот стрим уже идет
23) скрипт проверки где сейчас идут стримы
25) ??почистить лишние записи стримов в БД
28) обработчик сравнивающий дату начала стрима и начала записи
26) при рестарте сервера, должна быть очистка коллекции с живыми стримами + заполнение свежими
27) ручка - showlivestreams
27) ручка - showlivestreams/:id

приоритет 1:
для дашборда:
2) подписчиков и просмотры - разделить по разрядам
4) на фронте обрабатывать больше/меньше/не изм для просмотров и подписчиков, и количество дней
...
6) при отправке инфы в фронт переводить время в местное
7) перенести dateConverter(или использовать нативные методы?) в бэк
...
вопросы к Дену:
21) как часто обновлять подписчиков и просмотры и во сколько
26) при смене игры или заголовка приходит новый вебхук о начале трансляции. Если так получилось что начало стрима пропустили или сервер перезагрузился и начало стрима потеряно, можно записать стату стрима с момента нового вебхука, с пометкой "не с начала"
