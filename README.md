...
+ подкорректировать возвращаемый ответ, чтобы сформировать алерт. Пока приходит так:
{
    "refactObj": {
        "followers":[{"12/4/2020":652437}],
        "name":"SilverName",
        "twitchID":70075625,
        "views":[{"12/4/2020":115736042}],
        "maxOnline":0,
        "midOnline":{"value":0,"inDays":0},
        "_id":"5fca3d08069077a75548321b"
    },
    "addChannel": {
        "message":"в основной стэк добавлен канал SilverName(70075625)",
        "result":{
            "result":{"n":1,"ok":1},
            "connection":{"id":8,"host":"localhost","port":27017},
            "ops":[
                {
                    "followers":[{"12/4/2020":652437}],
                    "name":"SilverName",
            ...
            "insertedCount":1,
            "insertedId":"5fca3d08069077a75548321b",
            "n":1,
            "ok":1
        }
    },
    "subsWebHook":"запись успешно создана",
    "isStreamNow":"идет стрим, запущена запись статы"
}

+ удалить стримеров "Asmadey" - 83597658 и "SilverName" 70075625 + проверить в стате
...
+ у rxnexus у стрима 02.12.2020 10:58:18 нет зрителей - приходят null, вроде этот кейс уже учтен? учесть кейс при записи статы, возможно поэтому несколько записей статы не завершились. В стате лежит следующее:
maxViewers: 0
med50Viewers: null
midViewers: 0
minutes1Viewer: null