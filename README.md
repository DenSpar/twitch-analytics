...
3) сделал плашку "закрыт", на локхосте работает
4) если канал закрыт - на стр стримера вместо лого пустой блок и ссылка на канал не выводится
5) добавил в дашборд и стр. стримера последнюю инфу про подписчиков и просмотры *с последними датами

дальше:
+ поправить срипт записи подписчиков
+ вместо столбца "видео" - "Стримов за 7 д."(пока только переименовал) -> на дашборд надо отправлять кол-во стримов за 7д
+ для дашборда: переписать создание объекта в gitList. Чтобы объект на отправку формировался без спреда, а из прописанных полей
+ на дашборде у steel и rnexus почему-то нет ср-онлайн -> если был очень короткий стрим, то midViewers и med50Veiwers не наполняются -> учесть в recStreamStat и учесть наличие null в при подсчете ср midViewers за месяц
+ в getStreamer и в getMidAndMaxOnline есть функция getDaysDiff, если она понадобится для страницы стримера, то лучше вынести в отдельный модуль