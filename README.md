...
дальше:
+ вместо столбца "видео" - "Стримов за 7 д."(пока только переименовал), если приходит норм закоментировать функцию getVideosInfo
+ для дашборда: переписать создание объекта в getList. Чтобы объект на отправку формировался без спреда и удалений полей
...
+ в getStreamer и в getMidAndMaxOnline есть функция getDaysDiff, если она понадобится для страницы стримера, то лучше вынести в отдельный модуль. вынес в отдельный модуль в preparingStreamers4Send, заменить везде на путь