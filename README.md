...
02.11.20
1) заменить twitch-api-v5 на sendRequest и удалил twitch-api-v5

дальше:

фронт:
13) заменить twitch-api-v5 на sendRequest
++) переписать логику приложения, чтобы вся инфа формировалась на бэке и во фронт оправлялась в готовом виде

бэк:
13) заменить twitch-api-v5 на sendRequest
14) перенести dateConverter и videoTimeConverter перенести в бэк
15) запросы перенести в бэк
++) переписать логику приложения, чтобы вся инфа формировалась на бэке и во фронт оправлялась в готовом виде
16) импорты в бэке
...
4) сделать поля name, followers и views в streamers
+) скрипт ежедневного обновления кол-ва followers и views и имени
