...
7) fix recStreamStat и табл со стримами - если стрим был 1-2 мин, то maxViewers = null.  В таком случае midViewers и med50Viewers тоже = null + они не могут получить значение из maxViewers
...
5) написал обработку ответа от добавления нового стримера на фронте, теперь появляются соответствующие алерты
6) написал обработку ответа от удаления стримера на фронте, теперь появляются соответствующие алерты
7) fix recStreamStat и табл со стримами - проверить на rnexus(стрим от 02.12.2020 10:58:18)
