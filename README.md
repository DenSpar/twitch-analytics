...
проверить:
1) на стр сримера в описание "онлайн-зрителей" как выглядит null
2) на дашборде у steel и rnexus почему-то нет ср-онлайн

вопросы:
8) в ячейке длинны стрима показывать, когда появились первые зрители?
6) нужен ли скрипт длинны идущего стрима?
62) шрифты?

3) с учетом такой формы блока описания, переносить просмотры во вкладку "архивные видео"?
4) на странице стримера изменения выделять красным и зеленым?
5) не понял про "все стримеры". Это кнопка для возврата в дашборд? или нужен список стримеров в каком-то виде?
6) ссылка на канал открывает в новом окне?

потом:
66) сделать сортировку по столбцам
67) добавить временной диапазон для стримов
68) вместо столбеца видео "Стримов за 7 д."(пока только переименовал)
65) разделить table.css на table, dashboardTable и searchTable
5) для дашборда: переписать создание объекта в gitList. Чтобы объект на отправку формировался без спреда, а из прописанных полей
...
19) скрипт, обновления состояния стрима на фронте через запросы на бэк. Написать скрипт проверки стримов