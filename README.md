...
4) при записи статы стрима в БД, добавил обработчик сравнивающий дату начала стрима и начала записи. Если разница больше 5 минут появляется запись 'сбор статистики не с начала стрима'. Результат пока не видел
...
7) написал скрипты проверки, добавления и удаления записей из живых стримов

16.11.20
1) написал скрипт замены записи в БД живых стримов, если был записан старый стрим, пока не проверял
2) пофиксил ширину таблиц
3) добавил условие проверки стрима перед записью статы, еще не проверял
4) перенес условие проверки + добавил скрипт на добавление записи о стриме в БД, если таковой не было
5) написал скрипт, который при рестарте сервера очистки коллекции с живыми стримами + заполнение свежими + старт записи тех что идут. Еще не проверял


обновить и проверить результаты:
37) скрипт обновления записи в БД живых стримов
38) пофиксил ширину таблиц
36) скрипт проверки добавить перед записью стрима + добавил скрипт на добавление записи о стриме в БД, если таковой не было
26) скрипт, который при рестарте сервера очистки коллекции с живыми стримами + заполнение свежими + старт записи тех что идут


приоритет 0:
сбор статы со стримов:

25) ??почистить лишние записи стримов в БД

приоритет 1:
для дашборда:
5) переписать создание объекта, который будет отправляться в дашборд. Чтобы было без спреда, а точно прописанные поля для отправки
