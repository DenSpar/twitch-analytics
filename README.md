...
доделать:
1) сделать переход по клику с дашборда + получать id из url + изменять титул вкладки
2) при загрузке страницы стримера видно часть контента, отображать только целиком(завязать на лоадер)
3) разделительная полоса между блоками на странице стримера
4) может добавить длинну видео?