...
Обновить:
51) перед отправкой развернуть массив стримов
47) просмотры в табл видео, количество зрит в табл стримов - разделить по разрядам
48) даты начала стрима и начала записи статы в табл стримов - преобразовать даты
55) онлайн зрителей перенести в описание стримера: onlineViewers: {max:"1 101",middle:"438",inDays:"за 1 д."} -> переделать дашборд?

приоритет 1:
для страницы стримера:
бэк:
    57) при передаче инфы о стриме уменьшить колво полей(так-то мне нужно только колво зрителей) -> разделить по разрядам
    60) перенести videoTimeConverter
фронт:
    52) наверно пронумеровать игры в ячейке
    59) удалить getList из фронта
